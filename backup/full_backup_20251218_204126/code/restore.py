...
def detect_target_db_from_backup(backup_path: str) -> str:
    """Определяет целевую БД на основе имени файла бэкапа.
    
    Args:
        backup_path: Путь к файлу бэкапа
    
    Returns:
        str: Путь к целевой БД
    """
    backup_name = Path(backup_path).name
    
    # Если это referral_orders_*.db (без имени кабинета в середине), это первый кабинет
    if backup_name.startswith("referral_orders_") and backup_name.count("_") == 2:
        # Формат: referral_orders_TIMESTAMP.db
        return "referral_orders.db"
    
    # Если это referral_orders_<cabinet>_*.db, извлекаем имя кабинета
    if backup_name.startswith("referral_orders_") and backup_name.count("_") >= 3:
        # Формат: referral_orders_<cabinet>_TIMESTAMP.db
        parts = backup_name.replace(".db", "").split("_")
        # parts = ["referral", "orders", "<cabinet>", "TIMESTAMP"]
        if len(parts) >= 3:
            cabinet_name = parts[2]
            return f"referral_orders_{cabinet_name}.db"
    
    # По умолчанию возвращаем первую БД
    return "referral_orders.db"
...
def restore_database(backup_path: str,
                    ...
                    auto_detect_target: bool = True) -> bool:
    """
    Восстанавливает базу данных из бэкапа.
    
    Args:
        backup_path: Путь к файлу бэкапа
        target_db: Путь к целевой базе данных (если None и auto_detect_target=True, определяется автоматически)
        ...
        auto_detect_target: Автоматически определять целевую БД по имени файла бэкапа
    
    Returns:
        True если восстановление успешно, False в случае ошибки
    """
    # Автоматически определяем целевую БД, если не указана
    if target_db is None and auto_detect_target:
        target_db = detect_target_db_from_backup(backup_path)
        print(f"📋 Автоматически определена целевая БД: {target_db}")
    elif target_db is None:
        target_db = "referral_orders.db"
